...
function setImageEditRequestAttributes(span, params, uploadCallback) {
    return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        if (params.image &&
            ...
            if (imageUrl) {
                attributes[`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.1.content`] = JSON.stringify([
                    { type: "image_url", image_url: { url: imageUrl } },
                ]);
                attributes[`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.1.role`] = "user";
            }
        }
        Object.entries(attributes).forEach(([key, value]) => {
            if (value !== undefined) {
...
function setImageVariationRequestAttributes(span, params, uploadCallback) {
    return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        if (params.image &&
            ...
            const imageUrl = yield processImageInRequest(params.image, traceId, spanId, uploadCallback, 0);
            if (imageUrl) {
                attributes[`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.0.content`] = JSON.stringify([
                    { type: "image_url", image_url: { url: imageUrl } },
                ]);
                attributes[`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.0.role`] = "user";
            }
...
function setImageGenerationResponseAttributes(span, response, uploadCallback, instrumentationConfig, params) {
    return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        if (response.data && response.data.length > 0) {
            ...
            attributes[aiSemanticConventions.SpanAttributes.LLM_USAGE_COMPLETION_TOKENS] = completionTokens;
            ...
            if (instrumentationConfig === null || instrumentationConfig === void 0 ? void 0 : instrumentationConfig.enrichTokens) {
                try {
                    ...
                    if (estimatedPromptTokens > 0) {
                        attributes[aiSemanticConventions.SpanAttributes.LLM_USAGE_PROMPT_TOKENS] =
                    ...
                    attributes[aiSemanticConventions.SpanAttributes.LLM_USAGE_TOTAL_TOKENS] = completionTokens;
                }
            }
            else {
                attributes[aiSemanticConventions.SpanAttributes.LLM_USAGE_TOTAL_TOKENS] = completionTokens;
        ...
        if (response.data && response.data.length > 0) {
            ...
            if (firstImage.b64_json && uploadCallback) {
                try {
                    ...
                    attributes[`${aiSemanticConventions.SpanAttributes.LLM_COMPLETIONS}.0.content`] =
                        JSON.stringify([{ type: "image_url", image_url: { url: imageUrl } }]);
                    attributes[`${aiSemanticConventions.SpanAttributes.LLM_COMPLETIONS}.0.role`] = "assistant";
                }
                catch (error) {
...
class OpenAIInstrumentation extends instrumentation.InstrumentationBase {
    ...
    init() {
        const module = new instrumentation.InstrumentationNodeModuleDefinition("openai", [">=4 <6"], this.patch.bind(this), this.unpatch.bind(this));
        return module;
    }
    ...
    startSpan({ type, params, client, }) {
        ...
        const attributes = {
            [aiSemanticConventions.SpanAttributes.LLM_SYSTEM]: provider,
